import DashboardView from '@/views/dashboard/DashboardView.vue'
import AuthLayout from '@/views/dashboard/layout/DashboardLayout.vue'

import IndexUsers from '@/views/dashboard/users/IndexView.vue'
import CreateUser from '@/views/dashboard/users/CreateView.vue'
import EditUser from '@/views/dashboard/users/EditView.vue'

import CategoriesIndex from '@/views/dashboard/blog/categories/IndexView.vue'
import PostsIndex from '@/views/dashboard/blog/posts/IndexView.vue'
import type { RouteRecordRaw } from 'vue-router'

export const dashboardRoutes: readonly RouteRecordRaw[] = [
  {
    path: '/dashboard',
    component: AuthLayout,
    meta: {
      layout: 'dashboard',
      requiresAuth: true,
    },
    children: [
      {
        path: '',
        name: 'dashboard.home',
        component: DashboardView,
        meta: {
          title: 'Dashboard',
        },
      },
      {
        path: 'users',
        name: 'dashboard.users',
        component: IndexUsers,
        meta: {
          title: 'users',
        },
      },
      {
        path: 'users/create',
        name: 'dashboard.users.create',
        component: CreateUser,
        meta: {
          title: 'create users',
        },
      },
      {
        path: 'users/edit',
        name: 'dashboard.users.edit',
        component: EditUser,
        meta: {
          title: 'edit users',
        },
      },
      {
        path: 'categories',
        name: 'dashboard.categories',
        component: CategoriesIndex,
        meta: {
          title: 'Categories',
        },
      },
      {
        path: 'posts',
        name: 'dashboard.posts',
        component: PostsIndex,
        meta: {
          title: 'Posts',
        },
      },
    ],
  },
]
