...
const navLinks = [
  ...
  { name: 'nav.about', path: '/about' },
  {
    ...
    path: '/blog',
    submenu: [
      // { name: 'nav.categories', path: '/blog/categories' },
    ],
  },
...
const navDir = computed(() => (currentLocale.value === 'ar' ? 'rtl' : 'ltr'))
</script>
  ...
  <header class="fixed top-0 w-full shadow-md z-50 border-b border-b-muted bg-background">
    <div class="mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex gap-2 items-center">
          ...
          <DropdownMenu v-if="auth.isAuthenticated">
            <DropdownMenuTrigger as-child>
              <Button variant="outline" class="flex gap-3 items-center">
                <Avatar
                  class="h-8 w-8 transition-all duration-300 group-data-[state=collapsed]:h-6 group-data-[state=collapsed]:w-6"
                >
                  <AvatarImage :src="auth.user?.avatar || 'https://i.pravatar.cc/40'" />
            ...
            <DropdownMenuContent side="top">
              <DropdownMenuItem @click="goToProfile">
                <UserCircle class="me-3" /> <span>{{ $t('profile') }}</span>
              </DropdownMenuItem>

              <DropdownMenuItem @click="logout" class="flex items-center">
                <LogOut class="me-3" /> <span>{{ $t('sign_out') }}</span>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <div class="flex md:hidden justify-between items-center h-16 relative">
        ...
        <router-link v-if="!auth.isAuthenticated" to="/auth/login">
          <Button variant="outline">
            <LogIn />
            <span class="hidden md:block">{{ $t('login_title') }}</span>
          </Button>
        </router-link>
        <DropdownMenu v-else>
          <DropdownMenuTrigger as-child>
